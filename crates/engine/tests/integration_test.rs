...
async fn test_dry_run_with_manual_test_config() {
    ...
    let config: common::configs::FullEngineConfig =
        serde_yaml::from_str(&config_content).expect("Failed to parse engine.yaml");

    let request_content = tokio::fs::read_to_string(&request_path)
        .await
        .expect("Failed to read request.json");

    let request: common::models::RunRequest =
        serde_json::from_str(&request_content).expect("Failed to parse request.json");
